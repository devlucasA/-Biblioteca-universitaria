...
void heapify(Livro *arr, int n, int i) {
    int maior = i; // Inicializa o maior como raiz
    int esquerda = 2 * i + 1;
    int direita = 2 * i + 2;

    if (esquerda < n && arr[esquerda].ISBN > arr[maior].ISBN) // Verifica o filho esquerdo
        maior = esquerda;

    if (direita < n && arr[direita].ISBN > arr[maior].ISBN) // Verifica o filho direito
        maior = direita;
...
void merge(Livro *arr, int esquerda, int meio, int direita) {
    int n1 = meio - esquerda + 1;
    int n2 = direita - meio;

    Livro L[n1], R[n2]; // Arrays temporários para as duas metades
    ...
    while (i < n1 && j < n2) {
        if (L[i].ISBN <= R[j].ISBN) { // Compara pelo ISBN
            arr[k] = L[i];
            i++;
        } else {
            arr[k] = R[j];
